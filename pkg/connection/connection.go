...
func NewConnection(host string, port int, remoteCa string, cert tls.Certificate) (Connection, error) {
	block, _ := pem.Decode([]byte(remoteCa))
	if block == nil {
		return nil, fmt.Errorf("invalid ca specified")
	}
